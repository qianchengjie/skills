...
4. **可审查、可留证**：规范明确，reviewer 能依据决定性事实判断 `pass / failed / cannot-verify`，并说明判断依据。
...
以下内容不得作为 active rule：一次性任务要求、当前 bug 的临时背景、具体 workflow 的 artifact 格式、空泛最佳实践、单文件或单 PR 细节、无法明确符合或违反的判定依据的建议、已被现有规则覆盖的重复表达。
...
- 内容审查：由 agent 或 reviewer 核对规则格式、入库标准、正文是否完整承载有效规范及其语义影响，并记录判断依据。
