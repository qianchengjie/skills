...
`规则` 正文是规范语义的 Authority，完整表达义务、禁止、例外和必要证据义务。正文可以写在 `规则：` 同一行，也可以在后续缩进的段落或列表中展开；空行用于分段。`生效条件` 决定适用范围；规则级别只影响处置，不改变规则是否满足的事实。

说明和示例按需放在规则块内，辅助理解正文。reviewer 依据完整规范与决定性事实判断符合、违反或无法验证，并记录判断依据；证据载体由消费 workflow 指定。
...
未知 ID 会失败。active rule 的级别必须有效，生效条件和规范正文必须非空。
reader 校验必要字段、编号、路由和来源冲突，按 ID 返回完整规则块；规范正确性、
语义完整性及证据充分性由内容审查判断。
