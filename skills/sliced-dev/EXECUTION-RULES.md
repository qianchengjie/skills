...
general reviewer 以 `review-packages/<S-id>.md` 为主输入，只输出前三个 verdict 和 Findings 表；普通包不包含 `项目规则审查` 信息。首轮是 full review；若前三个 verdict 失败，不运行 rule-reviewer，先修复并写回本轮 `done` general review A*，再生成以该 A* 为直接基线的 incremental package。复核优先用 `followup_task` 恢复原 reviewer；只有原 reviewer 不可用时才新建 reviewer，并在 `门禁记录` 保留 fallback 原因与基线 A*。新 reviewer 也必须消费同一 incremental package，不得重新扫描整个任务。
...
只有切片 scope、任务目标、全局约束、Claims 契约或 task brief 实质改变时，才允许写 `AI Review：pending（full：<原因>）` 重建 full 基线。基线损坏、reviewer 更换、修复文件变多或新增验证证据都不构成 full reason。脚本只检查明示状态、引用、表格和枚举结构；不判断 full reason 的业务语义或 finding 分类是否正确。
...
- 修复后重跑失败门禁，重新生成 package，并按上一轮 A* 做 incremental AI Review；不得每轮重扫整个任务。
