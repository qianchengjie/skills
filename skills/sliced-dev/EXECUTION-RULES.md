...
| 类型 | 触发 | 问法 | 用户回答含义 |
|---|---|---|---|
| 拆分拷问选择 | 拆分拷问门禁（计划一致性预检通过后） | 先按 `SKILL.md`「拷问展示协议」输出当前可能需要确认的内容和推荐理由，再问“是否进入拷问？请回复：拷问 / 不拷问。” | 只选择澄清方式；只有 `拷问` / `不拷问` 有效 |
| 切片拷问选择 | 切片前拷问门禁（候选需确认 / 跨模块片审查前） | 先按 `SKILL.md`「拷问展示协议」输出当前可能需要确认的内容和推荐理由，再问“是否进入拷问？请回复：拷问 / 不拷问。” | 只选择澄清方式；只有 `拷问` / `不拷问` 有效 |
| 拷问收口 | 已进入拷问，且 agent 判断暂无更多拷问问题 | “是否结束拷问？可回复：结束拷问 / 继续拷问；如果还有其他问题，也可以直接提问。结束拷问只结束拷问方式，不关闭未决分叉。” | 只有 `结束拷问` 会收口；`继续拷问` 或直接提问都保持拷问态 |
| 分叉确认 | 用户明确不拷问，或拷问后仍需单点确认 | 按 `SKILL.md`「拷问展示协议」的具体问题格式，一次只问一个分叉结论 | 只确认口径 |
...
- `WRITE_TASK_BRIEF`：每轮派发前生成或刷新当前片 `task-briefs/<S-id>.md`，作为 implementer 的窄上下文入口；brief 必须渲染当前片 claims，以及已写回并关联的失败门禁或当前 General Review A* / open G* 修复依据；`项目规则审查：blocked` 时不得生成；需确认片必须先生成 brief，再发执行预告。
...
- `AI_REVIEW_PACKAGE_AND_REVIEW`：先用本轮 before / after workspace tree 和上一 `targetTree` 执行 `seal-target`，再从 sealed range 生成 package。General Review 按首次累计 `full`、finding-focused `repair`、发生 repair 后最终累计 `full` 推进；每个 TARGET 在 `项目规则审查：required` 时都生成累计 rule package 并派发 fresh rules-review v4 run。controller 只用最新最终 full 和当前 rules-review run 一次性写回四 verdict。
...
general reviewer 以 `review-packages/<S-id>.md` 为主输入，只输出当前阶段允许的结果；普通包不包含 `项目规则审查` 信息。每轮 general review 都使用 `spawn_agent(fork_turns: "none")` 新建 reviewer，禁止用 `followup_task` 复用原 reviewer。首次 `full` 完整审查 `BASE → TARGET1` 并输出前三个 verdict 和完整 `openFindings`；`repair` 只审查 `TARGETn-1 → TARGETn`，逐一裁决上一轮 finding 并报告 fix diff 新 finding，不输出或继承前三个 verdict；发生过 repair 后必须再执行 `BASE → TARGET_final` 的最终累计 `full`。新 reviewer 只消费本轮 package 和其中显式引用的直接上一轮 A*，不继承上一 reviewer 的会话记忆。

每个 sealed TARGET 在 `项目规则审查：required` 时都生成 `review-packages/<S-id>-rules.md`，并派发 fresh rule-reviewer 运行完整 rules-review v4 协议。规则包始终复制当前 sealed `baseTree`、`targetTree`、累计文件快照和 `BASE → TARGET` diff；每个新 TARGET 创建全新 run，完整审查全部当前 `reviewItems`，不携带旧 run、旧规则审查 A* 或旧 SHOULD 接受 D*，也不继承旧结果。controller 在 `AI Review 结论` 中写唯一安全的当前 `项目规则审查 runId`，并用新 A* 投影 fixed summary；非 `ready_for_merge` 时 A* 必须记录 `.rules-review-tmp/<runId>/response.md`。general review A* 与项目规则审查 A* 各自遵循自己的快照协议，不互相覆盖。

结构合法的正负结果都是有效审查结果，负结论不得通过重派洗掉。只有 reviewer 未返回、越界写文件或 final summary 无法归属本轮 package / run 时，才允许对同一输入最多 fresh 重派一次，禁止对 reviewer 使用 `followup_task`；仍失败则写 `AI Review：blocked（<原因>）` 并停止。`close-check` 会要求 general / rule package、当前 A*、rules-review dispatch 和 sealed range 的 `baseTree`、`targetTree`、累计文件快照及可选 `boundCommit` 一致。
...
package 是注意力收束视图，不是事实真源。允许针对 P0/P1 claim、具名风险、边界或证据缺口做 focused Git blob 读取 / `rg` / focused test，但禁止从当前文件、真实 index、`git status` 或同名路径重新构造审查范围。`review-packages/<S-id>-range.json` 是本轮封印身份；package 只能从其中的 `baseTree`、`targetTree`、tree entries 和 blobs 生成。对象缺失、本轮 patch 不能唯一应用、hunk 重叠、组合结果不等于 `workspaceAfterTree`，或 `HEAD != seedCommit` 时 fail closed，原 A* 与对应 rules-review run 立即失效。`review-packages/**`、`task-briefs/**`、`task-reports/**` 是临时输入，脚本会维护 `dev-plans/.gitignore` 的对应模式，并从 diff-check 和 package inventory 中排除；不建立 ref、synthetic commit 或长期恢复 anchor。

每轮 general reviewer 返回后，controller 必须新建 `done` A*，记录 `reviewType / previousReview / baseCommit / baseTree / seedCommit / targetTree / boundCommit / reviewPackageHash / openFindings`。`full` 还记录三个 verdict；`repair` 只记录 `Finding Results`，不得记录或继承三个 verdict。`reviewPackageHash` 必须原样取自本轮 reviewer final summary；每个 A* 都物化当前完整 `openFindings`，但只引用直接上一轮，不递归复用结果。

`repair` 必须对直接上一轮 `openFindings` 中每个旧 finding 恰好返回一次 `addressed` 或 `not_addressed`。当前开放集合只能机械派生为旧 finding 中的 `not_addressed` 加本轮 `Origin=repair-delta` 新 finding；不得静默丢失、复制或把其它范围发现混入 repair。repair 不对 `BASE → TARGETn` 做开放式完整审查。开放集合清零且此前发生过 repair 时，下一轮只能是最终累计 `full`；最终 full 发现新问题时再进入 repair，修复后再次 full。缺失直接前序、hash / tree 绑定不一致、finding 裁决不闭合或 Git object 不可用时一律 fail closed，不得用另一次 full 绕过坏状态。
...
项目规则语义审查必须在代码 commit 前完成并封印最终 rules-review v4 run。正式提交前先执行 `pre-commit-check`，要求 `HEAD == seedCommit`；不相等时当前 General Review A* 和对应 rules-review run 都失效，必须重新封印和审查。代码 commit 成功后不重做语义审查，而是立即用 `bind-target` 补充 sealed range 的可选 `boundCommit`，并对最终 rules-review run 执行 `bind-commit`；两处绑定都只验证对象类型为 commit 且 `boundCommit^{tree} == targetTree`，不验证父提交、祖先或 merge 拓扑。无代码变更且累计目标为空时，不创建空 commit，也不要求绑定。
...
- 每次派发返修前，先把失败硬门禁、当前 General Review A* / open G*，或当前项目规则审查 A* / `rulesReviewReport` 修复依据写回并关联，重新生成 task brief，再重置 task report；随后优先 `followup_task` 原 implementer，并明确要求最新 brief 覆盖旧上下文。只有 [IMPLEMENTER-SUBAGENT.md](IMPLEMENTER-SUBAGENT.md) 明示的不安全复用条件才 fresh fallback；既有授权内扩展执行 allowlist 仍复用原 implementer。
- 修复后重跑失败门禁，重新记录 `workspaceAfterTree` 并执行 `seal-target`；按直接上一轮 A* 生成 `repair` package，只复核旧 `openFindings` 和本轮 fix diff。开放 finding 清零后必须生成最终累计 `full` package；两种阶段都新建 general reviewer。
...
- 每个 done + `AI Review：passed` slice 必须有非空 task brief、`conclusion: ready-for-review` 的非空 task report、非空 review-package 和合法 `review-packages/<S-id>-range.json`；JSON report 必须 schema valid。当前 A* 必须是覆盖 `BASE → TARGET_final` 的 `full`；发生过 repair 时，缺少最终累计 full、旧 finding 未被唯一裁决或当前 `openFindings` 非空都拒收。
...
- `项目规则审查：required` 时，必须有唯一安全的当前 runId 选择器和当前最终 A*。`close-check` 从当前 sliced-dev skill root 安全解析受信任 rules-review validator，对当前仓库 `.rules-review-tmp/<runId>` 重跑真实 `--mode run`，不执行 A* 的 validation 展示命令；它拒绝不安全、缺失、symlink / 逃逸路径、validator / finalReview 失败，并比较真实 runId、recommendation、三个计数和仅 SHOULD recommendation 存在的 `shouldSetHash`。真实 dispatch 必须是 v4，`reviewRange.baseTree / targetTree / boundCommit`、`inputSnapshot`、changed units 及当前全部 `reviewItems` 的 shard 覆盖必须与 sealed range 和累计 rule package 一致；每个 TARGET 必须是独立 run，不接受 v3、旧 package 或结果继承字段。第四 verdict 默认必须等于 A* raw verdict；只有默认 SHOULD 完整 A/D/Evidence/token/confirmation 链可单独通过。`项目规则审查：not-applicable` 时，第四 verdict 必须为 `not-applicable` 且不得有选择器或适用规则 ID；`blocked` 阻塞 `上下文预检：ready`、`AI Review：passed` 和 `状态：done`。机器不判断 BASE 选择、hunk 业务归属、rule ID 是否该选、finding 是否正确、证据是否充分或用户确认是否真实。
...
整任务审查包使用首个执行切片的 `baseCommit` 到最终 `HEAD` 的累计已提交范围，用于检查全局约束是否被跨片绕开、切片交接是否一致、后续切片是否绕过前序非目标。生成前要求所有有代码范围的 sealed range 已绑定正式 commit。高风险任务提示转 `rules-review deep / cross-slice`，不得静默当成自动门禁通过。
...
- 执行型切片在 plan 内的 `Commit` 状态、提交后的 commit hash 或无变更说明；plan 的 `Commit` 仍只记录状态，不写 hash。commit hash 的持久机器真源是 sealed range 的 `boundCommit`；有规则审查绑定时，其 `reviewRange.boundCommit` 必须一致。完成报告可以展示该值。
- 偏离预告或未完成项。
...
- 首轮派发前用 `workspace-tree --seed <seedCommit>` 记录 `workspaceBeforeTree`；implementer 返回后再次记录 `workspaceAfterTree`。接收门禁先确认本轮 delta 文件集合与 task report、`允许修改` / `禁止修改` 一致，再用 `seal-target` 把本轮 patch 应用到上一 `targetTree` 并执行残余基线 patch 的双向组合验证。基线脏内容留在真实工作区，不进入 `targetTree`、package 或正式 commit。
...
- 正式提交前运行 `pre-commit-check`，要求 `HEAD == seedCommit`；不相等时当前审查全部失效并重新封印。只 stage `targetTree` 相对 `seedCommit` 的本片代码修改，不 stage `dev-plans/<date-slug>`，保留基线脏改动；若 patch 不能唯一隔离或必须带入基线内容，停止并报告。
...
- 本片产生代码 commit 时，提交后先绑定 sliced-dev sealed range；`项目规则审查：required` 时再绑定最终 rules-review run：

  ```bash
  node <sliced-dev-skill-dir>/scripts/dev-plan.mjs bind-target dev-plans/<date-slug> <S-id> --commit <commit>
  node <rules-review-skill-dir>/scripts/validate.js --mode bind-commit --dir .rules-review-tmp/<runId> --commit <commit>
  ```

  两处绑定都只核对对象类型为 commit 且 `commit^{tree} == targetTree`，不重做语义审查，也不检查父提交、祖先或 merge 拓扑；失败时不得把本片收口，也不得继续后续切片。无代码变更且累计目标为空时不执行绑定。
