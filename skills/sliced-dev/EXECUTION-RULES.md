...
- `WRITE_TASK_BRIEF`：生成当前片 `task-briefs/<S-id>.md`，作为 implementer 的窄上下文入口；需确认片必须先生成 brief，再发执行预告。
...
- `ACCEPT_IMPLEMENTER_REPORT`：控制器读取 subagent summary 和 `task-reports/<S-id>.md`，确认 `Implementer 结论：ready-for-review`、实际改动未越过 `允许修改` / `禁止修改`、且 subagent 未报告 blocked / 新分叉 / 风险升级；不通过则停止或重新派发，不进入硬门禁。
- `RUN_HARD_GATES`：执行 lint / type-check / test / `diff-check` 等确定性门禁。
...
- `USER_ACCEPTANCE`：AI Review 通过后，把当前片写为 `用户验收：pending` 并停下给用户验收；`passed` 或 `skipped（用户明确跳过）` 后才允许提交和标记 done，`issues` 进入本片有限修复或转 D 分叉。
...
`diff-check` 只做确定性边界检查：当前 git dirty files 是否落在 `允许修改`，是否命中 `禁止修改` 或 `禁止词`。已写入本片 `基线脏文件` 的既有脏文件会被跳过；`dev-plans/<date-slug>/plan.md`、`decisions.md`、`audits.md`、`ledger.md`、`review-packages/**`、`task-briefs/**`、`task-reports/**` 和自动维护的 `dev-plans/.gitignore` 会被跳过；rename 会同时检查新旧两个路径。它不会判断 helper 是否有业务语义，也不会判断 null 判断是否必要。
...
当前 `close-check` 不把 in-scope uncommitted business files 本身作为 blocker；只有越过 done slices 允许修改 union 或命中禁止修改时才失败。如果项目希望最终必须 clean，应由外层 git / CI gate 执行，或未来新增显式 strict mode；不要把当前 union dirty guard 误读为 strict clean。

满足任一条件时需要 Whole Review：切片数 > 1、任一切片 `风险：B/C`、任一切片有真实 `#### 接口契约`、任一切片关联 `D*`、任一切片 `允许修改` 跨模块。需要 Whole Review 时，`close-check` 要求顶部 `Whole Review：passed`，且 `review-packages/whole-task.md` 存在并非空；不需要时可写 `Whole Review：not-required`。
...
`Whole Review：passed` 不允许出现 `failed`、`cannot-verify-from-package`、`blocked` 或 `critical`；`Whole Review：blocked` 必须在 Evidence 说明阻塞证据。
...
- 提交前运行 `node <sliced-dev-skill-dir>/scripts/dev-plan.mjs diff-check dev-plans/<date-slug> <S-id>`，确保 dirty files 没越过本切片 `允许修改` / `禁止修改`；`diff-check` 已豁免本计划 `plan.md`、`decisions.md`、`audits.md`、`ledger.md`、`review-packages/**`、`task-briefs/**`、`task-reports/**` 和 `dev-plans/.gitignore`，执行期它们常驻脏是预期状态。
