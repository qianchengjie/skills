...
- `AI_REVIEW_PACKAGE_AND_REVIEW`：生成当前片 review-package，再按 [REVIEWER-SUBAGENT.md](REVIEWER-SUBAGENT.md) 派发 general reviewer subagent 输出三 verdict 和 Findings，并写回新的 general review A*；若 `项目规则审查：required`，生成 rule-review-package 并派发 rule-reviewer。controller 最终一次性写回四 verdict。
...
general reviewer 以 `review-packages/<S-id>.md` 为主输入，只输出前三个 verdict 和 Findings 表；普通包不包含 `项目规则审查` 信息。每轮 general review 都使用 `spawn_agent(fork_turns: "none")` 新建 reviewer，禁止用 `followup_task` 复用原 reviewer。首轮是 full review；若前三个 verdict 失败，不运行 rule-reviewer，先修复并写回本轮 `done` general review A*，再按下方 full 触发条件生成 full 或 incremental package。新 reviewer 只消费本轮 package，不继承上一 reviewer 的会话记忆。

若前三个 verdict 通过且 `项目规则审查：required`，生成 `review-packages/<S-id>-rules.md` 并派发 fresh rule-reviewer 运行完整 `rules-review` 协议。规则包排除旧规则审查 A* 和旧 SHOULD 接受 D*，历史只通过当前选择器投影的 `baseRunId` 连接。controller 在 `AI Review 结论` 中写唯一安全的 `项目规则审查 runId`，并用新 A* 投影本轮 fixed summary；非 `ready_for_merge` 时 A* 必须记录 `.rules-review-tmp/<runId>/response.md`，供下一轮 implementer 读取具体 finding。规则审查的部分修复仍必须生成新 runId、新 A*，不能沿用旧结论。general review A* 与项目规则审查 A* 各自遵循自己的快照协议，不互相覆盖。

结构合法的正负 verdict 都是有效审查结果，负结论不得通过重派洗掉。只有 reviewer 未返回、越界写文件或 final summary 无法归属本轮 package / run 时，才允许对同一输入最多 fresh 重派一次，禁止对 reviewer 使用 `followup_task`；仍失败则写 `AI Review：blocked（<原因>）` 并停止。`close-check` 会要求 rule review package 与当前 general review package 的业务变更范围一致，并核对真实 dispatch 的 selectedRuleRefs、changed units、input snapshot 和非空 changedFiles 没有脱离当前切片。
...
package 是注意力收束视图，不是事实真源。允许针对 P0/P1 claim、具名风险、边界或证据缺口做 focused Read / `rg` / focused test，但禁止运行 `git diff` / `git log` / `git status` 重新构造审查范围。package 的关联审计投影排除 General Review A*；incremental package 只在 `General Review 基线` 中内嵌一次直接基线，并额外包含 `General Review 模式` 和 `本轮修复索引`，其 Git Diff 是修复证据，不是重新全量扫描的授权。`review-packages/**`、`task-briefs/**`、`task-reports/**` 是临时输入，脚本会维护 `dev-plans/.gitignore` 的对应模式，并从 diff-check 和 package inventory 中排除；审计结论必须由控制器写回 plan / D/A。

每轮 general reviewer 返回后，controller 必须新建 `done` A* 记录 `模式 / 基线 / 条件性 Full reason / reviewPackageHash / 三 verdict / Findings`，并让 plan 前三 verdict Evidence 统一引用它。`reviewPackageHash` 必须原样取自本轮 reviewer final summary；incremental 快照保留基线所有 `G*`，旧 finding 只能显式更新 `Disposition`，不得静默消失。当前 A* 仅缺少 `reviewPackageHash` 时不得伪造回填，必须显式重新 full；基线缺失、多义、非 `done` 或存在其它结构损坏时，`review-package` fail-closed，不自动回退 full。旧 `issues / blocked` 计划没有该 A* 时，先一次性补建快照。

满足以下任一类条件时，controller 必须写 `AI Review：pending（full：<原因>）` 重新 full：
...
除此之外，只对开放 Findings 和本轮 fix diff 做 scoped re-review。新建 reviewer、已审范围内且可隔离的修复文件增加、新测试或新证据本身不构成 full reason。当前 A* 仅缺少 `reviewPackageHash` 时按上条重新 full；基线 A* 缺失、多义、非 `done` 或存在其它结构损坏时仍先 fail-closed 修复协议状态，不得用 full 绕过。协议闭合后仍无法证明可信演进链时重新 full。上述 full 判定属于 controller / reviewer 的语义责任；脚本只检查输入绑定、明示状态、引用、非占位 Full reason、表格和枚举结构，不判断 full reason、风险变化、fix diff 隔离或代码演进链是否可信。
...
增量复核中的新 finding 必须显式分流：修复 delta 直接引入的问题写 `Origin=repair-delta, Disposition=open`，进入当前有限修复循环；其它问题写 `Origin=late-discovered`，`critical / major` 写 `Disposition=blocked` 并停止，`minor` 写 `Disposition=parked` 并在完成报告保留残余风险，不进入当前修复循环。启用 `零已知缺陷收口` 时，当前切片引入或加重的 finding 不得 parked，必须修复或 blocked。
...
- 修复后重跑失败门禁，先判断是否命中重新 full 条件；命中则重建 full package，否则按上一轮 A* 生成 incremental package，只复核开放 Findings 和本轮 fix diff。两种模式都新建 general reviewer，不得每轮重扫整个任务。
