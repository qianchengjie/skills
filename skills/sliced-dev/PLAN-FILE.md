...
- `rule-review-package` 只在 `项目规则审查：required`、当前 TARGET 的最终累计 General full 已通过，且适用的用户验收为 `passed / skipped` 时生成，路径为 `review-packages/<S-id>-rules.md`；自动且未启用逐片验收的切片可在 General 后直接继续。默认 fresh full 包复制同一 Review Range、累计文件快照和 `baseCommit..headCommit` diff，但完全不包含 execution-time `项目规则审查` 块、`selectedRuleIds`、`notApplicable`、`规则获取` 或嵌有这些字段的 task brief，也不内联 general reviewer 三 verdict、旧项目规则 A* 或旧 SHOULD 接受 D*。rule-reviewer 必须基于最终 TARGET 和完整 active catalog 独立分类；除下一条的一跳返修复验外，每个新 TARGET 都创建全新 rules-review v8 run。同一 TARGET 只修正规则协议或输入工件时仅重做 rules-review，保留仍有效的 General 和用户验收。
- 当前 TARGET 由失败项目规则 A* 直接返修而来时，`rule-review-package` 默认尝试构造 sliced-dev 内部 repair verification。只允许引用完整、有效、同 BASE 的前序 full rules-review，要求当前 TARGET 是前序 TARGET 的直接子提交、规则身份未变且修复 delta 完整非空；规则身份机械等于 active catalog 的 index hash 与封印规则文件路径 / content hash。生成 `review-packages/<S-id>-rule-repair-task.json`，reviewer 把 strict JSON 写入 `review-packages/<S-id>-rule-repair-verification.json`，controller 运行 `rule-repair-check <planDir> <S-id>`。任务携带前序 `finding / cannot_verify`、前序 selected rules、前序 globally-not-applicable rules、完整 `previousTarget..currentTarget` delta 和封印代码输入；globally-not-applicable rules 只用于判断 applicability expansion，即是否有任一此类规则因 repair delta 在当前 TARGET 新变得 applicable，不在 repair verification 内审查新 applicable rule。它不携带或继承旧 `passed / observation`，不创建 rules-review run / `finalReview`，也不改变 rules-review v8。reviewer 逐项裁决旧问题、审查完整 delta 及语义影响，并把 `applicabilityExpansion.verdict` 明确写为 `none / detected / cannot_verify`；`none` 不声明完整 partition 双向不变。前序 selected rule 变得不适用是安全的 scope contraction，不要求重新分类或 fresh full。按需展开的未修改上下文只能从 `currentTargetCommit` 对应 Git tree / blob 读取，不得读取工作区、index 或当前 HEAD。共享 helper / type 改动必须检查所有可静态发现的调用方 / consumer，无法证明静态影响范围闭合时写 `scope_unbounded`。delta 夹带非前序失败项返修所需的功能改动时直接要求 fresh full。机器只检查 commit / 规则 / task hash 绑定、delta 文件闭合、applicability expansion / scope / disposition 枚举及派生门禁，不判断 expansion 语义、返修语义、影响范围或证据强度是否正确。
- repair verification 的机械结果固定为：`applicabilityExpansion.verdict=none`、全部旧问题 `addressed`、`scopeVerdict=bounded` 且无新 finding 时 `repaired / complete`；任一 `not_addressed` 或新 finding 时 `finding / repair`；applicability expansion `detected / cannot_verify`、任一其它 `cannot_verify` 或 `scope_unbounded` 时 `cannot_verify / fresh_full`。只有 expansion 明确为 `none` 时，当前 A* 才可投影前序 full 的 `reviewSelectedRuleRefs / reviewNotApplicable`；这两项仍表示前序 full 基线，其中 selected 可是当前范围的保守超集，不能解释为当前 TARGET 的完整 partition 已证明不变。构造条件不满足或返回 `fresh_full` 时不得在 constructor / validator 内静默降级，必须由 controller 明确运行 `rule-review-package <planDir> <S-id> --fresh-full-reason <原因>` 重新请求现有 fresh full。该路径仅一跳，不递归继承 repair verification；下一轮规则返修显式回到 fresh full。
