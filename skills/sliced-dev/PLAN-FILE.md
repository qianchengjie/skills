...
- `项目规则审查`：本片是否需要独立 rule-reviewer。`状态` 只允许 `required` / `not-applicable` / `blocked`；同时记录 `rules-review` 可用性、resolved `规则获取` 命令、`规则校验` 结果和适用规则 ID。无适用规则时写 `not-applicable`，并说明已检查规则仓但本片无适用 rule ID；有适用规则但 `rules-review` 不可用时也写 `not-applicable / unavailable`，保留 selectedRuleIds 与 `规则获取` 命令，`规则校验` 写 `skipped`，最终第四 verdict 的 Note 必须包含固定说明 `未执行独立项目规则审查`。
...
`整任务审查：passed` 时不得出现 `failed`、`cannot-verify-from-package`、`blocked` 或 `critical`；Evidence 必须非空。`整任务审查：blocked` 时必须保留完整表，阻塞说明写在正文说明中。`close-check` 会拒绝 `package-generated` 和 `blocked`，只有缺席或 `passed` 可继续收口；`passed` 仍必须通过整任务审查包结构校验。
...
- `AI Review：passed`：必须有完整四 verdict，且不得出现 `failed` / `cannot-verify-from-package` / `critical`
...
- `整任务审查：passed`：必须有完整五 verdict，且不得出现 `failed` / `cannot-verify-from-package` / `blocked` / `critical`
