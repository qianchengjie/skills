...
- 当前 TARGET 由失败项目规则 A* 直接返修而来时，`rule-review-package` 默认尝试构造 sliced-dev 内部 repair verification。只允许引用完整、有效、同 BASE 的前序 full rules-review，要求当前 TARGET 是前序 TARGET 的直接子提交、规则身份未变且修复 delta 完整非空；规则身份机械等于 active catalog 的 index hash 与封印规则文件路径 / content hash。生成 `review-packages/<S-id>-rule-repair-task.json`，reviewer 把 strict JSON 写入 `review-packages/<S-id>-rule-repair-verification.json`，controller 运行 `rule-repair-check <planDir> <S-id>`。任务只携带前序 `finding / cannot_verify`、前序 selected rules、完整 `previousTarget..currentTarget` delta 和封印代码输入；不携带或继承旧 `passed / observation`，不创建 rules-review run / `finalReview`，也不改变 rules-review v8。reviewer 逐项裁决旧问题并审查完整 delta 及语义影响；共享 helper / type 改动必须检查所有可静态发现的调用方 / consumer，无法证明静态影响范围闭合时写 `scope_unbounded`。delta 夹带非前序失败项返修所需的功能改动时直接要求 fresh full。机器只检查 commit / 规则 / task hash 绑定、delta 文件闭合、枚举和旧问题 disposition 完整性，不判断返修语义、影响范围判断或证据强度是否正确。
- repair verification 的机械结果固定为：全部旧问题 `addressed`、`scopeVerdict=bounded` 且无新 finding 时 `repaired / complete`；任一 `not_addressed` 或新 finding 时 `finding / repair`；任一 `cannot_verify` 或 `scope_unbounded` 时 `cannot_verify / fresh_full`。构造条件不满足或返回 `fresh_full` 时不得在 constructor / validator 内静默降级，必须由 controller 明确运行 `rule-review-package <planDir> <S-id> --fresh-full-reason <原因>` 重新请求现有 fresh full。该路径仅一跳，不递归继承 repair verification；下一轮规则返修显式回到 fresh full。
...
repair verification A* 的 `validation` 必须展示 `rule-repair-check <planDir> <S-id> => passed`，并额外且只写一个 `repairVerification: <review-packages/...-rule-repair-verification.json>`；`rulesReviewRunId`、`reviewSelectedRuleRefs` 和 `reviewNotApplicable` 投影前序 full 基线，`recommendation=ready_for_merge` 且三个计数均为 `0`，不得写 `rulesReviewReport` / `shouldSetHash`。`close-check` 重建当前 repair task、重验其前序 full v8 run，再核对直接父子 TARGET、BASE、规则身份、task hash、完整 delta 文件集、全部前序失败项 disposition、结果枚举、A* 和第四 verdict；它不把组合证据伪装成当前 TARGET 的新 full run，也不判断 reviewer 的语义影响分析是否正确。
