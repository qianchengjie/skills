...
repair verification A* 的 `validation` 必须展示 `rule-repair-check <planDir> <S-id> => passed`，并额外且只写一个 `repairVerification: <review-packages/...-rule-repair-verification.json>`；仅当 `applicabilityCheck.verdict=unchanged` 时，`rulesReviewRunId`、`reviewSelectedRuleRefs` 和 `reviewNotApplicable` 才投影前序 full 基线，`recommendation=ready_for_merge` 且三个计数均为 `0`，不得写 `rulesReviewReport` / `shouldSetHash`。`close-check` 重建当前 repair task、重验其前序 full v8 run，再核对直接父子 TARGET、BASE、规则身份、task hash、完整 delta 文件集、applicability 结论、全部前序失败项 disposition、结果枚举、A* 和第四 verdict；它不把组合证据伪装成当前 TARGET 的新 full run，也不判断 reviewer 的适用性或语义影响分析是否正确。
