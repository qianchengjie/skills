...
`提前全部拷问` / `全量提前拷问` 不新增顶部字段；进行中时用 `当前状态` 的 `下一步` 记录提前全量拷问及完成条件，进度由每片 `门禁` 承载。候选自动片预筛清楚时可写 `门禁：not-applicable`；需要拷问的切片收口后写 `门禁：grilled`。提前全量拷问一次只处理一个可执行切片，进入某片拷问时 `当前切片` 必须指向该片，同一计划中最多一个可执行切片为 `门禁：grilling`；续跑时先恢复 `grilling` 切片，没有 `grilling` 时再按顺序处理下一个 `pending-grill` 切片。所有可执行切片 `门禁` 都不再是 `pending-grill / grilling` 且无 open D 后，`下一步` 改回当前切片的正常执行流程。
