...
- 条件字段 `Commit`：只用于执行型切片（`not-started` / `blocked` / `in-progress` / `done`），值为 `待提交` / `已提交`；`split` / `skipped` 必须省略。
- `修复次数`：`当前次数/最大次数`，例如 `0/2`、`1/2`、`2/4`，统计本切片实际修改任务范围内文件的自动修复总次数，不按门禁分别计数。默认上限为 `2`；AI Review 已进入 `issues` 后可按有限修复规则一次性放宽到 `4`。单纯重跑 reviewer、重生成 package、补运行证据或修 review 协议工件不计次。次数用尽后任一门禁仍失败则停止，不继续自动修。
...
该约束缺席即表示未启用；写入时只允许固定值 `enabled`。启用后，所有执行型切片都必须完成 AI Review，A 类也不能使用 `AI Review：skipped`；本次变更引入或加重的所有 finding（无论 `must_fix` / `should_fix`）必须修复，所有 `cannot_verify` 必须补证清零，不接受用风险接受、claim waiver 或 follow-up 关闭缺陷。既有且未被本次变更加重的 observation 不自动扩入范围；合法失效的 claim 仍可使用 `waived`，但不能用它豁免 finding。规则审查 A* 还必须记录 `recommendation` 和 `issueSummary.mustFix / shouldFix / cannotVerify`；`close-check` 只校验固定 token 和这些结构化终态，不判断代码是否“完美”或 observation 是否应该升级。
...
- `review-package` 是 general reviewer 的注意力入口，不是事实真源；它不包含 `项目规则审查` 信息。若 P0/P1 claim、边界或证据无法从 package 判断，reviewer 必须 focused 回源检查，或输出 `cannot-verify-from-package`。
...
| --- | --- | --- | --- | --- |
| 需求符合性 | passed | not-applicable | Task Brief / Git Diff | 覆盖验收 |
| 切片边界 / 交接一致性 | passed | not-applicable | 上下文预检 / Git Diff | 遵守边界 |
| 代码质量 / AI 污染检查 | passed | not-applicable | Git Diff | 没有新增依赖 |
| 项目规则审查 | not-applicable | not-applicable | 上下文预检 / 项目规则审查 | 本片无适用项目规则 |
```

Evidence 必须非空；可填写 review-package 章节名、文件路径、A* 或固定不适用标记（`N/A` / `NA` / `not applicable` / `不适用`）。自然语言判断说明写入 Note。
...
`项目规则审查：required` 时，第四 verdict 不能是 `not-applicable`，Evidence 必须引用当前最终 A*；该 A* 必须至少包含 `selectedRuleIds`、`validation: <rules-review validate command> => passed`、`verdict`、`severity` 和 `summary`。启用 `零已知缺陷收口` 时，A* 还必须包含 `recommendation: ready_for_merge`，以及值均为 `0` 的 `issueSummary.mustFix`、`issueSummary.shouldFix`、`issueSummary.cannotVerify`。`项目规则审查：not-applicable` 时，第四 verdict 必须为 `not-applicable`，且上下文预检不得列出适用规则 ID。`项目规则审查：blocked` 时不得写 `上下文预检：ready` 或 `AI Review：passed`，也不得把切片标记为 `done`。`close-check` 只校验这个结构闭合，不判断 rule ID 是否该选、规则是否满足或 rules-review 映射是否准确。
...
- 切片 `修复次数`：`当前次数/最大次数`，最大次数只允许 `2` / `4`，当前次数不能超过最大次数；默认 `0/2`，只有 AI Review 已进入 `issues` 后才能把当前片从 `<当前>/2` 一次性放宽为 `<当前>/4`，放宽时保留当前次数并在 `门禁记录` 说明原因，AI Review 后续转为 `passed` 时保留 `/4`
...
`Commit` 只用于执行型切片，表示本片**代码**的提交状态，不表示 `dev-plans` 自身是否已提交，也不写最终 commit hash：未提交写 `待提交`，本片代码提交边界已收口写 `已提交`；`split` / `skipped` 不进入执行，必须省略该字段。`dev-plans` 记录走自己的独立 commit（默认收口 / 用户中途要求），不由切片 `Commit` 字段表达。最终 commit hash 只放在会话回复或外部提交记录中，不写回 plan；执行型切片无代码变更时不要创建空 commit，完成后仍写 `Commit：已提交`，并在 `验证备注` / 完成报告说明无可提交变更。
