...
- `not-required（<原因>）`：控制器明确不做 whole review，原因必须非占位，并能说明为什么不存在跨切片复核价值。
...
`## Whole Review 结论` 是整任务收口审查的结构化输出。`Whole Review：passed` 或 `Whole Review：blocked` 时必须填写完整固定表；`pending`、`package-generated`、`not-required（<原因>）` 可先保留占位。控制器判断不需要整审时写 `Whole Review：not-required（<非占位原因>）`。
...
- 顶部 `Whole Review`：`not-required（<原因>）` / `pending` / `package-generated` / `passed` / `blocked`
