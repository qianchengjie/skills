---
name: deep-rules-review
description: 当项目规则审查需要固定 commit、规则快照、分片执行、聚合结果与机器校验的完整审计链时使用。把固定代码 range 与规则来源消费为 `ruleSet → targets → applicabilityMatrix → reviewItems → reviewBatches → results → finalReview`，并用 validator 校验既有 v8 协议闭合。仅审查已提交代码；默认只读，不维护规则仓，不替代全量功能 QA。
disable-model-invocation: true
---

# 深度规范审查

`deep-rules-review` 承载既有 `rules-review` v8 协议，继续使用 `schemaVersion = 8` 以及既有 kind、schema 和 `.rules-review-tmp` 命名，避免把 Skill 迁移扩大成协议兼容性改造。代码 TARGET 只能是 Git commit；规则输入与代码 TARGET 独立。每个新的 TARGET 都创建全新 run，完整审查该 TARGET 的全部当前 `reviewItems`，不继承旧结果，也不把修复轮当作增量审查。
...
deep-rules-review run 是临时运行数据，不承诺跨会话、跨环境、跨天或长期恢复。
