...
2. 是当前确实要建立或改变、仍需独立确定的架构选择。

对照本任务适用的已有架构和本轮方案，合并等价表述，筛除已有决定的重复、已被其它决定充分确定的推论、
代码现状复述及与当前决定无关的结构事实。比较结构语义及适用范围，确认单元按下文划分。

再用 fresh Implementer 反事实复核：

> fresh Implementer 在理解适用的已有约束和本轮其它决定后，如果缺少这条选择，会不会更容易把代码放错位置、给错 owner、制造错误状态真源、穿透错误边界或形成错误依赖？

字段或 DTO 语义、业务行为、Ticket 范围、字段映射、具体文件 / 函数
...
候选混合了多类内容时，只提取其中能独立成立的结构选择。AI 负责判断内容归属和决策独立性；人的 `[x]`
只确认通过门禁后的架构设计，不负责判断一句话算不算架构。

每条决定写清所选结构关系及适用范围后即完成表达。必要背景用于消除该决定的实际歧义：缺少它时，
决定的对象、结构关系或适用范围存在不同合理解读。将消除歧义所需的最少信息直接补入对应决定，
含义明确后结束展开；新增结构选择按确认单元处理。
...
每个当前仍需独立确定的结构选择使用一条原子 checklist。保持适用的已有约束和本轮其它决定不变，
候选仍有需要另行确定的结构取舍时，才独立成条；独立性以当前方案中的选择为准。

```markdown
- [ ] Reception 是单聊完整过程的唯一状态真源。
```

各自需要确定的 owner、状态真源、边界或依赖分别成条。例如，命令写入路径和状态读取路径仍有各自的
结构取舍时分别确认。由同一能力约束充分确定的消费者行为，在实现中遵循该决定即可。
...
1. 按“定位文件”确定目标，读取当前内容及本任务适用的已有架构，建立本轮比较基线。
2. 拆开可独立判断的语义，按内容域门禁筛选、语义去重，并按确认单元整理独立选择与必要背景。
3. 完成 fresh Implementer 反事实复核，将通过筛选的内容按下表写入。
4. 写后重读文件，对照比较基线和本轮方案，复核每个候选是否有独立结构增量、是否与其它决定重复或已被其确定，以及背景是否确实消除歧义。背景中出现新的结构选择时，按确认单元处理。整理本轮候选后再次重读，直到当前 Delta 只保留筛选后的独立选择及必要背景。
5. 存在 `[ ]` 时，只展示当前 Architecture Delta 及必要上下文，等待人明确确认。写入请求、早先讨论和方向认可均不能替代对写入后当前 Delta 的确认；即使被要求“直接改完继续”，也保留 `[ ]` 并展示。
6. 应用确认前再次重读文件。如展示后 Delta 已变化，重新展示并等待人确认当前内容；内容一致时，按下表只处理本次明确确认的单元。
7. 文件中仍有任何 `[ ]` 时，明确返回“Architecture 尚未全部确认”；不允许用默认认可、进度压力或实现已完成替代确认。
