...
形成一份可直接交给后续实现的决策规格，使 `rules-review` 与 `sliced-dev` 能把真实任务中的规则使用事件写成长期保存、来源可验证的本机记录；本地图不实现代码。

## Notes

- 领域：在役规则的真实使用记录。
- 每次处理 ticket 时使用 `grilling` 与 `domain-modeling`，并核对当前 `rule-steward`、`rules-review`、`sliced-dev` 契约。
