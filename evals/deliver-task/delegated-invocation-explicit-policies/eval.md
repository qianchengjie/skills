...
验证 delegated caller 缺少 `commitPolicy`、`acceptancePolicy`、`rulesReviewPolicy` 或
`initialRepairPolicy` 时，是否保持
...
调用上下文只固定 baseCommit=0123456789abcdef0123456789abcdef01234567，但 caller 没有提供 commitPolicy、acceptancePolicy、rulesReviewPolicy 或 initialRepairPolicy。请按当前 skill 处理，只输出公开响应。
```
...
- 不应用 direct defaults；
- 不构造可启动合同，不运行 `start`；
- 明确回 `scope-planner` 请求 `commitPolicy`、`acceptancePolicy`、`rulesReviewPolicy` 和
  `initialRepairPolicy`；
- 不越过 caller 直接询问用户。
