...
验证 direct caller 在用户和固定调用上下文都没有提供提交与验收策略时，是否仍构造完整合同，并使用
`commitPolicy=required`、`acceptancePolicy=not-required` 两个固定默认值而不询问用户。
...
- 返回完整 `deliver-task.task.v1` JSON；
