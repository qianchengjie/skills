...
合同要求 upstream acceptance；实现与验证已完成，Git target 已固定，General 与适用的
Rules concern 已通过首次 discovery 或最终 clean Review Wave 闭合。controller 因当前 target
尚无验收结果返回 needs-upstream/user-acceptance。此后代码、执行路径边界、Git identity、
