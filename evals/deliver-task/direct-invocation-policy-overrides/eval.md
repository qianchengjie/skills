...
验证 direct caller 是否让用户明确的提交、验收、Rules Review 与首次 repair 要求覆盖固定默认值，构造
`commitPolicy=forbidden`、`acceptancePolicy=required`、`rulesReviewPolicy=not-required`、
`initialRepairPolicy=auto` 的完整合同。
...
“$deliver-task 把 scripts/format.mjs 的默认并发数从 4 改成 6；保持 CLI 参数兼容；不要修改 package-lock.json。不要提交，改完让我确认；这次不需要 rules-review。首次 review 有 findings 后不用等我批准，自动修。”
...
- 使用 `caller.kind=direct`、`commitPolicy=forbidden`、`acceptancePolicy=required`、
  `rulesReviewPolicy=not-required`、`initialRepairPolicy=auto`；
...
本用例只验证 direct caller 对显式提交、验收、Rules Review 与首次 repair 要求的公开 policy 选择，不判断 policy 静默分支、
