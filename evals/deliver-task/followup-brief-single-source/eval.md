...
- 若随后输出 follow-up JSON，`message` 只提供当前 task directory、完整 `Reread / Unchanged` 声明与 task-owned evidence 定位；本场景中只要求重读最新 task brief，并声明 `task.json`、`execution.json` 与适用 Architecture 未变化；
...
- 在已有完整变化事实时仍要求原 Implementer 重读未变化的 Task、Execution 或适用 Architecture，或让其自行发现 delta；
- 把最新 task brief 当成可覆盖 `task.json` 的合同真源，或把额外业务语义作为同级硬约束；
- 为此新建返修 ledger、额外状态机或新的任务报告字段。

## Evaluation boundary

本用例只验证返修派发是否保持 `task.json > task-brief.md` 的 authority 层级、是否按完整变化声明只刷新已变化输入，以及 message 是否只做定位；不判断新增业务取舍最终应进入哪个既有 task-owned 载体，也不要求机器检查业务内容。Evaluator 只依据 subject 的公开回复和公开工具动作判定，不要求、推断或保存隐藏思考过程。本文件定义一次独立调用，不定义 repetitions、RED/GREEN 编排、隔离、重试、聚合或结果文件格式。
