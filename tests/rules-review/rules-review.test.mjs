...
async function materializePassingRun(dispatchFile) {
  ...
  writeJson(path.join(runDir, "shards/B001.json"), passedShard(dispatch));
  try {
...
test("commit、provided tree 与 branch 解析得到不可变历史范围且不写 seedCommit", async (t) => {
  const root = createRepository();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test("task 复制固定 range 和 inputSnapshot，消费者拒绝缺失 Git object", async (t) => {
  const root = createRepository();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  fs.writeFileSync(path.join(root, "src/main.js"), "export const main = 2;\n");
  const file = createDraft(root);
...
test("bind-commit 只要求 commit tree 等于 targetTree，不检查提交拓扑", async (t) => {
  const root = createRepository();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  fs.writeFileSync(path.join(root, "src/main.js"), "export const main = 2;\n");
  const file = createDraft(root);
