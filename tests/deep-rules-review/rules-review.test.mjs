...
const validator = path.join(repoRoot, "skills/deep-rules-review/scripts/validate.js");
...
function currentProtocolVersion() {
  return readJson(
    path.join(repoRoot, "skills/deep-rules-review/schemas/dispatch.schema.json"),
...
test("语义切片分别审查，同一 batch 的 finding 按显式 rootCause 合并并保留全部证据组", async (t) => {
  ...
  const shardSchema = readJson(path.join(repoRoot, "skills/deep-rules-review/schemas/shard.schema.json"));
  ...
  const schema = readJson(path.join(repoRoot, "skills/deep-rules-review/schemas/final-review.schema.json"));
...
test("render-handoff 生成不含本机路径的可转发修复说明", async (t) => {
  ...
  assert.match(handoff, /# deep-rules-review 修复交接/);
...
test("runId 只接受 UTC 时间、rr 标记和 8 位小写十六进制随机后缀", async (t) => {
  ...
  for (const schemaFile of [
    ...
    const schema = readJson(path.join(repoRoot, "skills/deep-rules-review/schemas", schemaFile));
...
test("v7 工件与旧 incremental 字段明确拒绝，v8 工件通过", async (t) => {
  ...
  assert.equal(
    fs.existsSync(path.join(repoRoot, "skills/deep-rules-review/schemas/retry-task.schema.json")),
    false,
  );
});
...
test("文档声明 commit-only、每 TARGET fresh run 与临时生命周期", () => {
  const skill = fs.readFileSync(path.join(repoRoot, "skills/deep-rules-review/SKILL.md"), "utf8");
  const reviewer = fs.readFileSync(path.join(repoRoot, "skills/deep-rules-review/references/subagent-all-aspects.md"), "utf8");
