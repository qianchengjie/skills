...
需求、任务、研究与实验记录承载项目知识，结构决定和规则还会影响多次开发。维护时需要考虑内容是否有效、依据能否找到，以及材料之间的关系和依赖。

需要管理这些知识与材料时，可以使用 [knowledge-steward](knowledge-steward.md)。它根据具体问题维护内容、索引和引用，判断材料去留，保留结论的依据与适用范围。现有 Spec、研究笔记、任务和项目文档都可以继续承载知识；长期组织方式尚未确定时，先维护现有材料。内容更正、材料处理和 Git 写操作分别按相应授权执行。

这一入口可以用于探索、研究、问题排查、任务交付或日常维护。任务完成后，调用方也可以按需要选择它，处理已确认变化、引用或材料保留问题。已有材料满足需要时，说明结论即可结束；需要新决定或专门维护时，交给相应环节。
