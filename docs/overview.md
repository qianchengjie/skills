...
本仓库的 [task-steward](task-steward.md) 判断已有需求是否需要拆分，不限定来源或载体。先确认候选 task 的承诺、成果和验证边界，再判断具体拆分收益是否足以抵消耦合与流程成本。通过形成候选、逐项复核、集合与证据分层复核、局部迭代调整边界；每项区分需首次证明的契约与交互和可依赖的前置结论，集合核对需求覆盖及局部与组合验证的承接。逐项与集合复核通过，且找不到有充分依据、值得采用的重要调整时，当前划分收敛。
