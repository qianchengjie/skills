...
本仓库的 [task-steward](task-steward.md) 判断已有需求是否需要拆分，不限定来源或载体。先由自然实现闭环、核心状态或模块耦合、生命周期、真实依赖顺序及后续设计重开关系形成任务边界，再根据关键未知、发布或回滚风险及拆分造成的工程问题修正边界，最后检查任务承诺、结果能否验证与审查，以及整体需求覆盖。验证与审查只检查边界，不负责生成边界；正常交接成本不否定已成立的自然实现边界。发现问题时局部迭代；逐项与集合检查通过、重要候选边界已评估且没有待处理的边界问题时，当前划分收敛。
