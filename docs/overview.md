...
本仓库的 [task-steward](task-steward.md) 判断已有需求是否需要拆分，不限定来源或载体。先确认候选 task 的承诺、成果和验证边界，再判断具体拆分收益是否足以抵消耦合与流程成本。信息充分且无拆分收益时，标为 `ready`，保留原需求并说明判断；有合格边界且值得拆时，标为 `needs-split`，形成平级 task；缺决定性信息时标为 `cannot_verify`，保留缺口。已有落实授权时，沿用项目约定或调用方指定载体写入明确部分，核对原需求覆盖，区分局部落实与整体拆分完成。
